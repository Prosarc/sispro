				...
				/*INICIO DEL MENU1 PARA PROSARC*/
					...
					->addIf(in_array(Auth::user()->UsRol, Permisos::TODOPROSARC) || in_array(Auth::user()->UsRol2, Permisos::TODOPROSARC),
						(Menu::new()
							...
							->addIf(in_array(Auth::user()->UsRol, Permisos::LISTAGENERADORES) || in_array(Auth::user()->UsRol2, Permisos::LISTAGENERADORES),(Link::toUrl('/generadores', '<i class="fa fa-industry"></i> <span>'.  __('adminlte::message.MenuGenerClien').'</span>')))
					...
					->addIf(in_array(Auth::user()->UsRol, Permisos::VEHICULOS) || in_array(Auth::user()->UsRol2, Permisos::VEHICULOS),
						(Menu::new()
							...
							->addParentClass('treeview')
							->add(Link::toUrl('/vehicle', '<i class="fas fa-list-alt"></i> '. __('adminlte::message.MenuVehiclelist')))
							->add(Link::toUrl('/vehicle-mantenimiento', '<i class="fas fa-tools"></i> '. __('adminlte::message.MenuMantVehic')))
							->addClass('treeview-menu')
						)
					)
					...
					->addIf(in_array(Auth::user()->UsRol, Permisos::TODOPROSARC) || in_array(Auth::user()->UsRol2, Permisos::TODOPROSARC),
						(Menu::new()
							...
							->addParentClass('treeview')
							->add(Link::toUrl('/crm/dashboard', '<i class="fas fa-chart-line"></i> Dashboard'))
							->add(Link::toUrl('/crm/oportunidades', '<i class="fas fa-lightbulb"></i> Oportunidades'))
							->add(Link::toUrl('/crm/interacciones', '<i class="fas fa-comments"></i> Interacciones'))
							->addClass('treeview-menu')
						)
					)
					...
					->addIf(in_array(Auth::user()->UsRol, Permisos::TODOPROSARC) || in_array(Auth::user()->UsRol2, Permisos::TODOPROSARC),
						(Menu::new()
							...
							->addParentClass('treeview')
							->addIf(in_array(Auth::user()->UsRol, Permisos::SERVICIOS) || in_array(Auth::user()->UsRol2, Permisos::SERVICIOS), (Link::toUrl('/solicitud-servicio', '<i class="fas fa-file-invoice"></i> <span>'.  __('adminlte::message.MenuServTitleSidebar').'</span>')))
					...
					// ->addIf(in_array(Auth::user()->UsRol, Permisos::SERVICIOS) || in_array(Auth::user()->UsRol2, Permisos::SERVICIOS),(Link::toUrl('/solicitud-servicio', '<i style="font-size: 1.2em; color: #aa66cc;" class="fas fa-people-carry"></i> <span>'. __('adminlte::message.MenuServTitle').'<span>')))
