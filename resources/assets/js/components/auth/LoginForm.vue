<template>
 <form method="post" @submit.prevent="submit" @keydown="clearErrors($event.target.name)">
  <div class="alert alert-success text-center" v-show="form.succeeded" id="result"> {{ __('adminlte_message.loggedin') }} <i class="fa fa-refresh fa-spin"></i> {{ __('adminlte_message.entering') }}</div>
  ...
  <div class="row">
   <div class="col-xs-8">
    <div class="checkbox icheck">
     <label>
      <input type="checkbox" name="remember" v-model="form.remember"> {{ __('adminlte_message.remember') }}
     </label>
    </div>
   </div>
   <div class="col-xs-4">
    <button type="submit" class="btn btn-primary btn-block btn-flat" v-text="__('adminlte_message.buttonsign')" :disabled="form.errors.any()"><i v-if="form.submitting" class="fa fa-refresh fa-spin"></i></button>
   </div>
  </div>
 </form>
</template>
  ...
  export default {
    ...
    computed: {
      placeholder: function () {
        if (this.name === 'email') return this.__('adminlte_message.email')
        return this.__('adminlte_message.username')
      },
      type: function () {
    ...
    methods: {
      submit () {
        this.form.post('/login')
          ...
          .catch(error => {
            console.log(this.__('adminlte_message.loginerror') + ':' + error)
          })
      },
      adddomain: function () {
