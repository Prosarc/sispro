<template>
 <form method="post" @submit.prevent="submit" @keydown="clearErrors($event.target.name)">
  ...
  <div class="row">
   ...
   <div class="col-xs-8">
    <button type="submit" class="btn btn-primary btn-block btn-flat" :disabled="form.errors.any()"><i v-if="form.submitting" class="fa fa-refresh fa-spin"></i> {{ __('adminlte_message.sendpassword') }}</button>
   </div><!-- /.col -->
   <div class="col-xs-2">
   </div><!-- /.col -->
  </div>
  ...
  methods: {
    submit () {
      this.form.post('/password/email')
       ...
       .catch(error => {
         console.log(this.__('adminlte_message.sendpassword') + ':' + error)
       })
    },
    clearErrors (name) {
