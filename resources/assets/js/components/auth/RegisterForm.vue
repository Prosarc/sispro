<template>
 <form method="post" @submit.prevent="submit" @keydown="clearErrors($event.target.name)">
  <div class="alert alert-success text-center" v-show="form.succeeded" id="result"> {{ __('adminlte_message.registered') }} <i class="fa fa-refresh fa-spin"></i> {{ __('adminlte_message.entering') }}</div>
  <div class="form-group has-feedback " :class="{ 'has-error': form.errors.has('name') }">
   <input type="text" class="form-control" :placeholder="__('adminlte_message.fullname')" name="name" value="" v-model="form.name" autofocus/>
  ...
  <div class="form-group has-feedback" :class="{ 'has-error': form.errors.has('email') }">
   <input type="email" class="form-control" :placeholder="__('adminlte_message.email')" name="email" value="" v-model="form.email"/>
    ...
    <label>
     <div class="checkbox_register icheck">
      <label data-toggle="modal" data-target="#termsModal">
       ...
       <a href="#" :class="{ 'text-danger': form.errors.has('terms') }" v-text="__('adminlte_message.conditions')"></a>
      </label>
     </div>
    </label>
   </div>
   <div class="col-xs-4 col-xs-push-1">
    <button type="submit" class="btn btn-primary btn-block btn-flat" :disabled="form.errors.any()" v-text="__('adminlte_message.register')"><i v-if="form.submitting" class="fa fa-refresh fa-spin"></i> </button>
   </div>
  </div>
    ...
    methods: {
      submit () {
        this.form.post('/register')
          ...
          .catch(error => {
            console.log(this.__('adminlte_message.registererror') + ':' + error)
          })
      },
      clearErrors (name) {
